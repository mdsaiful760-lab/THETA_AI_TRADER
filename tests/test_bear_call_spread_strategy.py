"""Tests for the deterministic bear-call-spread strategy plugin."""
...
import strategy.bear_call_spread_strategy as bear_call_module
...
from strategy.bear_call_spread_strategy import (
...
def snapshot(*, deltas: bool = True, reverse: bool = False):
    """Build a fresh NIFTY chain with positive bear-call-spread credit."""
    contracts = [
        contract(24300, OptionType.PE, -0.40, bid=120.0, ask=122.0),
        contract(24300, OptionType.CE, 0.60, bid=280.0, ask=282.0),
        contract(24400, OptionType.PE, -0.30, bid=80.0, ask=82.0),
        contract(24400, OptionType.CE, 0.45, bid=200.0, ask=202.0),
        ...
        contract(24600, OptionType.CE, 0.25, bid=58.0, ask=60.0),
        contract(24700, OptionType.PE, -0.60, bid=260.0, ask=262.0),
        contract(24700, OptionType.CE, 0.18, bid=38.0, ask=40.0),
        contract(24800, OptionType.PE, -0.70, bid=320.0, ask=322.0),
        contract(24800, OptionType.CE, 0.10, bid=18.0, ask=20.0),
        contract(24900, OptionType.PE, -0.75, bid=380.0, ask=382.0),
        contract(24900, OptionType.CE, 0.06, bid=10.0, ask=12.0),
        contract(25000, OptionType.PE, -0.80, bid=450.0, ask=452.0),
        contract(25000, OptionType.CE, 0.04, bid=6.0, ask=8.0),
    ]
    if reverse:
...
def context(**overrides: object) -> StrategyContext:
    ...
    values: dict[str, object] = {
        "correlation_id": "bear-call-correlation",
        "as_of": AS_OF,
        "snapshot": snapshot(),
        "tags": {
...
def strategy(
    configuration: BearCallSpreadConfiguration | None = None,
) -> BearCallSpreadStrategy:
    ...
    return BearCallSpreadStrategy(
        configuration or default_bear_call_spread_configuration(), framework
    )
...
def test_enter_happy_path_and_signal_mapping() -> None:
    ...
    assert result.strike_selection.short_call_strike == Decimal("24600.0")
    assert result.strike_selection.long_call_strike == Decimal("24800.0")
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"iv_rank": "55", "trend_direction": "BEARISH", "trend_strength": "0.35"},
...
def test_time_premium_and_greeks_abstentions() -> None:
    ...
    bounded = BearCallSpreadConfiguration(entry_time_window=TimeWindow(time(9), time(11)))
    assert strategy(bounded).evaluate_recommendation(context()).reasons == (
        "BCS.TIME.OUTSIDE_ENTRY_WINDOW",
    )
    assert strategy(
        BearCallSpreadConfiguration(minimum_premium=Decimal("1000"))
    ).evaluate_recommendation(context()).reasons == ("BCS.PREMIUM.BELOW_MINIMUM",)
    assert strategy().evaluate_recommendation(
        context(snapshot=snapshot(deltas=False))
    ).reasons == ("BCS.GREEKS.MISSING",)


def test_typed_context_and_serialization_round_trip() -> None:
    """Typed evidence works and entry recommendation JSON is canonical."""
    base = context(tags={})
    typed = BearCallSpreadContext(
        base,
        MarketRegimeEvidence("TRENDING_DOWN", AS_OF),
        EventRiskEvidence(False, AS_OF),
        TrendEvidence(AS_OF, "BEARISH", Decimal("0.35")),
        Decimal("55"),
    )
    recommendation = strategy().evaluate_bear_call_spread(typed)
...
def test_configuration_validation_and_concurrent_calls() -> None:
    ...
    with pytest.raises(ValueError, match="CFG-BCS"):
        BearCallSpreadConfiguration(short_target_delta=Decimal("0.50"))
    with pytest.raises(ValueError, match="CFG-BCS"):
        BearCallSpreadConfiguration(long_target_delta=Decimal("0.30"))
    with pytest.raises(ValueError, match="CFG-BCS"):
        BearCallSpreadConfiguration(supported_underlyings=frozenset())
    instance = strategy()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"maximum_bullish_trend_strength": Decimal("1.5")},
...
def test_all_configuration_invariants_fail_closed(kwargs: dict[str, object]) -> None:
    ...
    with pytest.raises(ValueError, match="CFG-BCS"):
        BearCallSpreadConfiguration(**kwargs)  # type: ignore[arg-type]
...
def test_remaining_recommendation_gates_and_public_aliases() -> None:
    ...
    ).reasons == ("BCS.METRIC.NON_FINITE",)
    assert instance.evaluate_recommendation(
        context(tags={"regime_tag": "TRENDING_DOWN", "iv_rank": "55"})
    ).reasons == ("BCS.TREND.MISSING",)
    assert strategy(
        BearCallSpreadConfiguration(supported_underlyings=frozenset({"SENSEX"}))
    ).evaluate_recommendation(context()).reasons == ("BCS.UNDERLYING.UNSUPPORTED",)
    stale = replace(context().snapshot.freshness, age_seconds=6.0)
    assert instance.evaluate_recommendation(
        context(snapshot=replace(context().snapshot, freshness=stale))
    ).reasons == ("BCS.SNAPSHOT.STALE",)
...
def test_wing_policies_and_pricing() -> None:
    ...
    fixed = BearCallSpreadConfiguration(
    ...
    delta_policy = BearCallSpreadConfiguration(
    ...
    configured = BearCallSpreadConfiguration(
    ...
    ask_credit = BearCallSpreadConfiguration(
...
def test_window_and_serialization_error_boundaries() -> None:
    ...
    with pytest.raises(ValueError, match="BCS.SERIALIZATION.INVALID"):
...
def test_numeric_helpers_and_boundary_greps() -> None:
    ...
    with pytest.raises(ValueError):
        bear_call_module._decimal(object())
    assert bear_call_module._parse_expiry("not-an-expiry") is None
    tokenized = replace(
        contract(24600, OptionType.CE, 0.25, bid=58.0, ask=60.0),
        instrument_token=99,
    )
    assert bear_call_module._instrument_id(tokenized) == "99"
...
def test_liquidity_filters_and_empty_chain() -> None:
    ...
    tight = BearCallSpreadConfiguration(maximum_spread_width=Decimal("0.10"))
    assert strategy(tight).evaluate_recommendation(context()).reasons == (
        "BCS.STRIKE.NO_ELIGIBLE_SHORT",
    )
    oi_floor = BearCallSpreadConfiguration(minimum_open_interest=50000)
    ...
    assert strategy()._select(empty, AS_OF.date()) == "BCS.CHAIN.MISSING"
    bad_expiry = replace(
        snap,
    ...
    assert strategy()._select(bad_expiry, AS_OF.date()) == "BCS.CHAIN.INCOMPLETE"
...
def test_risk_metrics_edges_and_event_sink() -> None:
    ...
    expensive = contract(24600, OptionType.CE, 0.25, bid=90.0, ask=100.0)
    cheap = contract(24800, OptionType.CE, 0.10, bid=1.0, ask=2.0)
    assert instance._risk_metrics(selection, expensive, cheap) == (
        "BCS.RISK.NON_POSITIVE_MAX_LOSS"
    )
    zero_short = contract(24600, OptionType.CE, 0.25, bid=1.0, ask=2.0)
    zero_long = contract(24800, OptionType.CE, 0.10, bid=90.0, ask=100.0)
    assert instance._risk_metrics(
        entered.strike_selection, zero_short, zero_long
    ) == "BCS.PREMIUM.BELOW_MINIMUM"
    ...
    publishing = BearCallSpreadStrategy(
        default_bear_call_spread_configuration(), framework, event_sink=sink
    )
    sealed = publishing.evaluate_recommendation(context())
    assert sealed.state is EntryRecommendationState.ENTER
    assert "strategy.bear_call_spread.entered_candidate" in sink.topics
    broken = BearCallSpreadStrategy(
        default_bear_call_spread_configuration(), framework, event_sink=BrokenSink()
    )
...
def test_illiquid_quotes_and_helper_branches() -> None:
    ...
    crossed = contract(24600, OptionType.CE, 0.25, bid=60.0, ask=58.0)
    ...
    assert bear_call_module._clamp(Decimal("-1"), Decimal("0"), Decimal("1")) == Decimal(
        "0"
    )
    snap = snapshot()
    ...
    dte_blocked = BearCallSpreadConfiguration(minimum_dte=30, maximum_dte=30)
    assert strategy(dte_blocked)._select(snap, AS_OF.date()) == "BCS.CHAIN.INCOMPLETE"
    fixed_mismatch = BearCallSpreadConfiguration(
        wing_selection_policy=WingSelectionPolicy.FIXED_WIDTH,
        target_wing_width=Decimal("350"),
    )
    assert (
        strategy(fixed_mismatch)._long_candidates(
            [item for item in snap.option_chain.contracts if item.option_type is OptionType.CE],
            Decimal("24500"),
            contract(24600, OptionType.CE, 0.25, bid=58.0, ask=60.0),
            Decimal("0.10"),
        )
    ...
    no_target = BearCallSpreadConfiguration(
        wing_selection_policy=WingSelectionPolicy.FIXED_WIDTH
    )
    ...
    assert malformed_trend.reasons == ("BCS.METRIC.NON_FINITE",)
