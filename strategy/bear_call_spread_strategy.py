"""Deterministic bear-call-spread strategy recommendation plugin.
...
_SCHEMA_VERSION = "1.0"
_RISK_STATEMENT = (
    "DEFINED_RISK: bear call spread maximum loss is finite and wing-capped."
)
_PASS_REGIMES = frozenset({"TRENDING_DOWN", "BEARISH", "MEAN_REVERTING", "RANGE_BOUND"})
_BULLISH_DIRECTIONS = frozenset({"BULLISH", "TRENDING_UP"})
...
class WingSelectionPolicy(str, Enum):
    """Long-call / wing selection policy."""
...
class EntryRecommendationState(str, Enum):
    """Outcome of the bear-call-spread entry evaluation."""
...
class TimeWindow:
    ...
    def __post_init__(self) -> None:
        ...
        if self.start >= self.end:
            raise ValueError(
                "CFG-BCS-011: TimeWindow must be a same-day increasing interval."
            )
        try:
            ZoneInfo(self.timezone)
        except Exception as exc:
            raise ValueError("CFG-BCS-011: TimeWindow timezone is invalid.") from exc


@dataclass(frozen=True)
class BearCallSpreadContext:
...
@dataclass(frozen=True)
class BearCallSpreadConfiguration:
    """Immutable bear-call-spread suitability and selection policy."""
    ...
    maximum_bullish_trend_strength: Decimal = Decimal("0.55")
    ...
    def __post_init__(self) -> None:
        ...
        decimals = (
            ...
            self.maximum_bullish_trend_strength,
        ...
        if any(value is not None and not value.is_finite() for value in decimals):
            raise ValueError("CFG-BCS-001: Decimal configuration must be finite.")
        short_targets = (
            self.short_target_delta,
        ...
        if any(not (_ZERO < abs(value) < Decimal("0.50")) for value in short_targets):
            raise ValueError("CFG-BCS-004: short target deltas must be within (0, 0.50).")
        if not (_ZERO < abs(self.long_target_delta) < abs(self.short_target_delta)):
            raise ValueError(
                "CFG-BCS-012: long_target_delta must be within (0, short_target_delta)."
            )
        ...
        if not (_ZERO < long_call < short_call):
            raise ValueError(
                "CFG-BCS-005: long targets must be strictly below short targets."
            )
        if not (_ZERO <= self.minimum_iv_rank <= _HUNDRED):
            raise ValueError("CFG-BCS-001: minimum_iv_rank must be within [0, 100].")
        if not (_ZERO <= self.maximum_bullish_trend_strength <= _ONE):
            raise ValueError(
                "CFG-BCS-013: maximum_bullish_trend_strength must be within [0, 1]."
            )
        if self.maximum_spread_width is not None and self.maximum_spread_width <= _ZERO:
            raise ValueError("CFG-BCS-010: maximum_spread_width must be positive.")
        if not (_ZERO < self.maximum_relative_spread_width <= _ONE):
            raise ValueError(
                "CFG-BCS-001: maximum_relative_spread_width must be within (0, 1]."
            )
        if self.minimum_premium < _ZERO:
            raise ValueError("CFG-BCS-001: minimum_premium must be non-negative.")
        if (
            self.short_delta_selection_tolerance < _ZERO
            or self.long_delta_selection_tolerance < _ZERO
        ):
            raise ValueError("CFG-BCS-001: delta tolerances must be non-negative.")
        if (
            self.short_delta_selection_tolerance >= Decimal("0.50")
            or self.long_delta_selection_tolerance >= Decimal("0.50")
        ):
            raise ValueError("CFG-BCS-001: delta tolerances must be below 0.50.")
        if self.minimum_open_interest < 0 or self.minimum_volume < 0:
            raise ValueError("CFG-BCS-001: liquidity floors must be non-negative.")
        if self.max_snapshot_age_seconds <= 0 or self.iv_rank_lookback_observations <= 0:
            raise ValueError("CFG-BCS-001: configured counts must be positive.")
        if self.minimum_dte < 0 or self.maximum_dte < self.minimum_dte:
            raise ValueError("CFG-BCS-009: invalid DTE bounds.")
        if self.contract_multiplier <= _ZERO:
            raise ValueError("CFG-BCS-001: contract_multiplier must be positive.")
        for width in (self.target_wing_width, self.minimum_wing_width, self.maximum_wing_width):
            if width is not None and width <= _ZERO:
                raise ValueError("CFG-BCS-001: wing widths must be positive when set.")
        if (
            ...
            raise ValueError(
                "CFG-BCS-001: maximum_wing_width must be at least minimum_wing_width."
            )
        if self.minimum_liquidity_score is not None and not (
            _ZERO <= self.minimum_liquidity_score <= _ONE
        ):
            raise ValueError(
                "CFG-BCS-001: minimum_liquidity_score must be within [0, 1]."
            )
        if not isinstance(self.wing_selection_policy, WingSelectionPolicy):
            raise ValueError("CFG-BCS-001: invalid wing selection policy.")
        if not isinstance(self.premium_price_policy, PremiumPricePolicy):
            raise ValueError("CFG-BCS-001: invalid premium price policy.")
        if self.scoring_profile_name != "PREMIUM_SELLING":
            raise ValueError("CFG-BCS-014: only PREMIUM_SELLING is supported.")
        normalized = frozenset(
            item.strip().upper() for item in self.supported_underlyings if item.strip()
        )
        if not normalized:
            raise ValueError("CFG-BCS-007: supported_underlyings must not be empty.")
...
def default_bear_call_spread_configuration() -> BearCallSpreadConfiguration:
    ...
    return BearCallSpreadConfiguration()


@dataclass(frozen=True)
class BearCallSpreadStrikeSelection:
...
@dataclass(frozen=True)
class BearCallSpreadRiskMetrics:
    ...
    short_call_credit: Decimal
    long_call_debit: Decimal
    contract_multiplier: Decimal
    risk_statement: str = _RISK_STATEMENT


@dataclass(frozen=True)
class BearCallSpreadRecommendation:
    ...
    strategy_id: str
    as_of: datetime
    strike_selection: BearCallSpreadStrikeSelection | None
    risk_metrics: BearCallSpreadRiskMetrics | None
...
class BearCallSpreadStrategy(BaseStrategy):
    """Evaluate injected evidence for a defined-risk bear call credit spread."""
    ...
    def evaluate(self, context: object) -> object:
        ...
        if isinstance(context, (StrategyContext, BearCallSpreadContext)):
    ...
    def _evaluate_result(
        self, supplied: StrategyContext | BearCallSpreadContext
    ) -> BearCallSpreadEvaluationResult:
        ...
        if snapshot.freshness.age_seconds > self._configuration.max_snapshot_age_seconds:
            return self._failure(context, "BCS.SNAPSHOT.STALE", EntryRecommendationState.REJECT)
        local_time = (
        ...
        if not (window.start <= local_time < window.end):
            return self._failure(
                context, "BCS.TIME.OUTSIDE_ENTRY_WINDOW", EntryRecommendationState.ABSTAIN
            )
        if regime is None:
            return self._failure(context, "BCS.REGIME.MISSING", EntryRecommendationState.REJECT)
        tag = regime.tag.strip().upper()
        if tag == "HIGH_VOLATILITY_CRISIS":
            return self._failure(context, "BCS.REGIME.CRISIS", EntryRecommendationState.REJECT)
        if tag not in _PASS_REGIMES:
            return self._failure(context, "BCS.REGIME.UNSUITABLE", EntryRecommendationState.ABSTAIN)
        if event is not None and event.adverse:
            return self._failure(context, "BCS.EVENT.ADVERSE", EntryRecommendationState.ABSTAIN)
        ...
        if iv_rank is None and self._configuration.require_iv_rank:
            return self._failure(context, "BCS.IV_RANK.MISSING", EntryRecommendationState.REJECT)
        if iv_rank is not None and (not iv_rank.is_finite() or not _ZERO <= iv_rank <= _HUNDRED):
            return self._failure(context, "BCS.METRIC.NON_FINITE", EntryRecommendationState.REJECT)
        if iv_rank is not None and iv_rank < self._configuration.minimum_iv_rank:
            return self._failure(context, "BCS.IV_RANK.LOW", EntryRecommendationState.ABSTAIN)
        selection_data = self._select(snapshot, context.as_of.date())
        if isinstance(selection_data, str):
            reject_codes = {
                "BCS.CHAIN.INCOMPLETE",
                "BCS.CHAIN.MISSING",
                "BCS.GREEKS.MISSING",
                "BCS.STRUCTURE.INVALID_GEOMETRY",
            }
            state = (
        ...
        short_call, long_call, expiry, spot, dte = selection_data
        selection = self._strike_selection(snapshot, spot, expiry, dte, short_call, long_call)
        metrics_or_code = self._risk_metrics(selection, short_call, long_call)
        if isinstance(metrics_or_code, str):
            state = (
                EntryRecommendationState.REJECT
                if metrics_or_code == "BCS.RISK.NON_POSITIVE_MAX_LOSS"
                else EntryRecommendationState.ABSTAIN
            )
        ...
        bundle = self._factor_bundle(
            tag, iv_rank or _ZERO, trend_strength, short_call, long_call, selection, metrics, event
        )
        score = self._scoring_framework.score(
            ScoreRequest("bear_call_spread", bundle, self._configuration.scoring_profile_name)
        )
        ...
        result = BearCallSpreadEvaluationResult(
    ...
    def _evaluate_trend(
        ...
        if trend is None:
            if self._configuration.require_trend_evidence:
                return "BCS.TREND.MISSING", EntryRecommendationState.REJECT
            return None
        direction = (trend.direction or "").strip().upper()
        if direction in _BULLISH_DIRECTIONS:
            return "BCS.TREND.STRONG_BULLISH", EntryRecommendationState.ABSTAIN
        ...
        if not strength.is_finite() or not (_ZERO <= strength <= _ONE):
            return "BCS.METRIC.NON_FINITE", EntryRecommendationState.REJECT
        if direction == "" and strength >= self._configuration.maximum_bullish_trend_strength:
            return "BCS.TREND.STRONG_BULLISH", EntryRecommendationState.ABSTAIN
        return None
    ...
    def _resolve(
        ...
        if isinstance(supplied, BearCallSpreadContext):
            return (
    ...
    def _validate_context(self, context: StrategyContext) -> str | None:
        if context.snapshot is None:
            return "BCS.SNAPSHOT.MISSING"
        if not isinstance(context.snapshot, MarketSnapshot):
            return "BCS.CONTEXT.INVALID"
        ...
        if underlying not in self._configuration.supported_underlyings:
            return "BCS.UNDERLYING.UNSUPPORTED"
        try:
    ...
    def _select(
        ...
        if not contracts:
            return "BCS.CHAIN.MISSING"
        try:
            spot = _decimal(snapshot.underlying.last_price)
        except ValueError:
            return "BCS.CHAIN.INCOMPLETE"
        ...
        for (_, expiry), group in sorted(
            ...
            calls = [item for item in group if item.option_type is OptionType.CE]
            if any(item.delta is None for item in calls) and self._configuration.require_greeks:
                missing_greeks = True
            shorts = self._short_candidates(calls, spot, self._short_target)
            if not shorts:
                continue
            short_call = shorts[0]
            longs = self._long_candidates(calls, spot, short_call, self._long_target)
            if not longs:
                continue
            long_call = longs[0]
            if _instrument_id(short_call) == _instrument_id(long_call):
                return "BCS.STRUCTURE.INVALID_GEOMETRY"
            short_strike = _decimal(short_call.strike)
            long_strike = _decimal(long_call.strike)
            ...
            if abs(_decimal(long_call.delta)) >= abs(_decimal(short_call.delta)):
                continue
            dte = (expiry - observation_date).days
            return short_call, long_call, expiry, spot, dte
        if missing_greeks:
            return "BCS.GREEKS.MISSING"
        if not groups:
            return "BCS.CHAIN.INCOMPLETE"
        return "BCS.STRIKE.NO_ELIGIBLE_SHORT"
    ...
    def _long_candidates(
        ...
        for contract in contracts:
            if contract.option_type is not OptionType.CE or contract.delta is None:
                continue
    ...
    def _risk_metrics(
        self,
        selection: BearCallSpreadStrikeSelection,
        short_call: OptionContractSnapshot,
        long_call: OptionContractSnapshot,
    ) -> BearCallSpreadRiskMetrics | str:
        short_credit = self._price_short(short_call)
        long_debit = self._price_long(long_call)
        net_credit = short_credit - long_debit
        if net_credit <= _ZERO:
            return "BCS.PREMIUM.BELOW_MINIMUM"
        if net_credit < self._configuration.minimum_premium:
            return "BCS.PREMIUM.BELOW_MINIMUM"
        wing = selection.wing_width
        if wing <= net_credit:
            return "BCS.RISK.NON_POSITIVE_MAX_LOSS"
        multiplier = self._configuration.contract_multiplier
        max_profit = net_credit * multiplier
        max_loss = (wing - net_credit) * multiplier
        if max_loss <= _ZERO:
            return "BCS.RISK.NON_POSITIVE_MAX_LOSS"
        short_otm = _clamp(_ONE - abs(selection.short_call_delta), _ZERO, _ONE)
        ...
        return BearCallSpreadRiskMetrics(
            net_credit,
            max_profit,
            max_loss,
            "DEFINED_RISK",
    ...
    def _factor_bundle(
        ...
        long_call: OptionContractSnapshot,
        selection: BearCallSpreadStrikeSelection,
        metrics: BearCallSpreadRiskMetrics,
        event: EventRiskEvidence | None,
    ) -> FactorInputBundle:
        spreads = (
            self._relative_spread(short_call) + self._relative_spread(long_call)
        ) / Decimal("2")
        liquidity = max(_ZERO, _HUNDRED * (_ONE - spreads))
        short_error = abs(abs(selection.short_call_delta) - self._short_target)
        long_error = abs(abs(selection.long_call_delta) - self._long_target)
        greek_quality = max(
    ...
    def _signal(
        self, context: StrategyContext, recommendation: BearCallSpreadRecommendation
    ) -> TradingSignal:
        ...
        if entered and selection is not None and recommendation.risk_metrics is not None:
            metadata = {
                ...
                "leg0_instrument_id": selection.short_call_instrument_id,
                "leg1_instrument_id": selection.long_call_instrument_id,
                "leg0_side": "SELL",
                "leg1_side": "BUY",
        ...
        return TradingSignal(
            ...
            as_of=context.as_of,
            action=action,
            direction=SignalDirection.BEARISH if entered else SignalDirection.UNKNOWN,
            strategy_id=self.metadata.strategy_id,
            strategy_version=self.metadata.version,
            strategy_family=self.metadata.strategy_family,
            confidence=SignalConfidence(
                float(score), confidence_band_for_score(float(score)), "bear_call_spread"
            ),
            market=market,
    ...
    def _publish(self, result: BearCallSpreadEvaluationResult) -> None:
        sink = self._event_sink
        if sink is None:
            return
        topic = {
            EntryRecommendationState.ENTER: "strategy.bear_call_spread.entered_candidate",
            EntryRecommendationState.ABSTAIN: "strategy.bear_call_spread.abstained",
            EntryRecommendationState.REJECT: "strategy.bear_call_spread.rejected",
...
def _plugin_config_for(configuration: BearCallSpreadConfiguration) -> StrategyPluginConfig:
    """Build immutable BaseStrategy metadata from bear-call-spread policy."""
    ...
    try:
        ...
        metrics = (
            BearCallSpreadRiskMetrics(
                ...
                _decimal(metrics_raw["short_call_credit"]),
                _decimal(metrics_raw["long_call_debit"]),
        ...
        return BearCallSpreadRecommendation(
        ...
        raise ValueError("BCS.SERIALIZATION.INVALID") from exc


__all__ = [
    "BearCallSpreadConfiguration",
    "BearCallSpreadContext",
    "BearCallSpreadEvaluationResult",
    "BearCallSpreadRecommendation",
    "BearCallSpreadRiskMetrics",
    ...
    "TrendEvidence",
    "WingSelectionPolicy",
    "default_bear_call_spread_configuration",
    "from_json",
    "to_json",
]
