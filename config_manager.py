...
class ConfigManager:
    ...
    DEFAULT_CONFIG = {
        ...
        # ----------------------------------------------------
        # POSITION SIZING — LEGACY COMPATIBILITY
        # ----------------------------------------------------
    ...
    def validate_setting(
        ...
        if section == "risk_budget":
            ...
            if key == "allocation_mode":
                normalized = str(value).upper()

                aliases = {
                    "FIXED": "FIXED",
                    "FIXED_TRADE": "FIXED",
                    "FIXED_TRADE_ALLOCATION": "FIXED",
                    "INTELLIGENT": "INTELLIGENT",
                    "SMART": "INTELLIGENT",
                    "DYNAMIC": "INTELLIGENT",
                    "INTELLIGENT_DYNAMIC": "INTELLIGENT",
                }

                if normalized not in aliases:
                    raise ValueError(
                        f"{name} must be FIXED or INTELLIGENT"
                    )
            ...
            if key == "max_trades_per_day":
                return self._validate_integer(
                    value, name, minimum=1, maximum=100
                )

            if key == "confidence_scaling_enabled":
                return self._validate_boolean(value, name)

            if key == "minimum_setup_score":
                return self._validate_number(
                    value, name, minimum=0.0, maximum=100.0
                )

            if key == "max_single_trade_daily_risk_pct":
                return self._validate_number(
                    value, name, minimum=1.0, maximum=100.0
                )

            if key == "intelligent_reference_trades":
                return self._validate_integer(
                    value, name, minimum=1, maximum=100
                )
            ...
            if key in (
                ...
                return self._validate_number(
                    value, name, minimum=0.0, maximum=5.0
                )
    ...
    def _validate_relationships(
        ...
        # ----------------------------------------------------
        # RISK BUDGET RELATIONSHIPS
        # ----------------------------------------------------

        risk_budget = config["risk_budget"]

        low_multiplier = float(
            risk_budget["confidence_multiplier_low"]
        )
        medium_multiplier = float(
            risk_budget["confidence_multiplier_medium"]
        )
        high_multiplier = float(
            risk_budget["confidence_multiplier_high"]
        )
        exceptional_multiplier = float(
            risk_budget["confidence_multiplier_exceptional"]
        )
        ...
        if not (
            ...
            raise ValueError(
                "risk_budget confidence multipliers must be ordered: "
    ...
    def get_risk_budget_config(self):
        """
        Convenience method for RiskBudgetAllocator.
        """
